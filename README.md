...
- **Создание заметок без агента:** Кнопка «Просто заметка» и Command Palette («Новая заметка», Cmd/Ctrl+K) — заметка сразу в корне без LLM
- **Офлайн-режим:** IndexedDB кеширует дерево и заметки; при отсутствии сети — чтение из кеша; изменения в очередь, синхронизация при подключении
- **Настройки LLM:** Модалка с проверкой подключения к серверу и API ключу
...
APK работает офлайн: при первом запуске нужно указать URL сервера; данные кешируются локально и доступны без интернета. Подробности — в [mobile/README.md](mobile/README.md).
